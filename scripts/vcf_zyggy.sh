...
cat "$vcf_in"  | vcf2bed | grep  '1/1' | bedtools map -c 2 -o count -a "$windows" -b - | awk '{print$0"\tHOM"}' > "$bed_out";
cat "$vcf_in"  | vcf2bed | grep  '0/1' | bedtools map -c 2 -o count -a "$windows" -b - | awk '{print$0"\tHET"}' >> "$bed_out";
